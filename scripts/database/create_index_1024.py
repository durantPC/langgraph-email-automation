...
# 使用 1024 维的模型
# 常见 1024 维模型：Qwen/Qwen3-Embedding-4B, BAAI/bge-m3 (实际是1024维)
...
try:
    ...
    if actual_dim != 1024:
        print(f"   ⚠️  警告: 模型实际维度为 {actual_dim}，不是预期的 1024")
        print(f"   将使用实际维度创建数据库目录: db_{actual_dim}")

    # 创建数据库
    db_path = f"db_{actual_dim}" if actual_dim != 1024 else "db_1024"
    print(f"\n3. 创建向量数据库到: {db_path}")
